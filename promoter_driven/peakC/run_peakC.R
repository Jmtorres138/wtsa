...
proj.dir <- "/home/wellgen/jtorres/projects/promoter-driven/"
plot.dir <- "/home/wellgen/jtorres/projects/promoter-driven/plots/"
...
find_chrom <- function(genename){
	probe.df <- fread(proj.dir%&%"capture_probes.txt")
...
read_r_capfile <- function(genename){
	...
	fname <- genename %&% "_eQTL_Capture_R_analysis.txt"
	rdir <- "/t1-data1/WTSA_Dev/ddownes/Diabetes_Tract/eQTL_Capture/Analysis/Stat_Package/eQTL_Capture_gfc/R_files/"
...
make_peakc_mat <- function(cap.df, viewpoint, sampname="condB_1_norm",window=1000e3){ 
...
make_expdata_list <- function(cap.df, view.point, window=1000e3, samp.vec){ #=c("condB_1_norm","condB_2_norm","condB_3_norm")){
	capclist <- list()
	i = 0
...
check_sigfrags_snps <- function(df,chrom,sig.fragments){
	...
	index.df <- filter(index.df,CHR==chrom)
	if (dim(index.df)[1] > 0){
...
plot_frag_over_cellmean<- function(genename,chrom,df,sig.fragments,viewpoint,window=100e3){
	...
	index.df <- select(index.df,one_of("CHR","START","RSID"))
	index.df <- index.df[!duplicated(index.df$RSID),]
	index.df <- filter(index.df,CHR==chrom,START>=xlow,START<=xhigh)
	snp.y <- rep(0,dim(index.df)[1])
	index.df <- cbind(index.df,snp.y)
...
sig_interactions <- function(genename, n.exp=3, sample.vec=c("condB_1_norm","condB_2_norm","condB_3_norm")){
