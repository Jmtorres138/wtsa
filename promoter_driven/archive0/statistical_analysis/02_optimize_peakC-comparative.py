...
# out_dir+"peakC-comparative_abscutoff-100_win11.txt"
...
def enrich(fname,abscutoff,win):
	temp_name = "abscutoff"+str(abscutoff)+"_win"+str(win)
	fin = open(fname,'r')
	fin.readline() # header
...
def main():
	if os.path.exists(out_file)==False:
		...
		fout.write("\t".join(["abscutoff","window","gwas.count","p.val"])+"\n")
	else:
		fout = open(out_file,'a')
	absc = fname.split("abscutoff-")[1].split("_win")[0]
	win = fname.split("win")[1].split(".txt")[0]
	gcount = enrich(fname,absc,win)[0]
	pval = enrich(fname,absc,win)[1]
	fout.write("\t".join([absc,win,str(gcount),str(pval)])+"\n")
	fout.close()
