...
prepare_input_list <- function(bg.df,vp,region.size=1e6,keep.cols=c(4,5,6)){
  ...
  # Note in union bedgraph, columns 4,5,6 correspond to Endos whereas 7,8,9 are Blymph
...
get_peakc_df <- function(id,peakc="sample1"){
  #peakc can be sample1, compared, or sample2 
...
write.table(x=na.omit(inter.df2),
            file=out.dir %&% "peakC-modeled-default_Blymph.txt",sep="\t",quote=F,row.names=F)
res.df2 <- build_results_df(na.omit(inter.df2))
write.table(x=res.df2,file=out.dir %&% "peakC-modeled-default_Blymph_withBaits.txt",sep="\t",quote=F,row.names=F)
...
write.table(x=res.df2,file=out.dir %&% "peakC-comparative_withBaits.txt",sep="\t",quote=F,row.names=F)
