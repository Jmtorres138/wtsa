...
credset_plot <- function(id,experiment,plotwin=5e5,
  ...
  if (promrefsnps==TRUE){
    ...
    s2 <- filter(ref.gwas.df,V1==params$Chr,
                 ...
                 V2<=params$Frag_stop+(plotwin))
    if (dim(s1)[1]>0){
      plt <- plt + geom_segment(data=s1,aes(x=V2,xend=V2),
                                y=1.15,yend=1.05,arrow=arrow(),color=viridis(20)[6])
    }
    if (dim(s2)[1]>0){
      plt <- plt + geom_segment(data=s2,aes(x=V2,xend=V2),
                                y=1.12,yend=1.02,arrow=arrow(),color=viridis(20)[15])
    }
  }
  plt = plt + theme_bw()
  return(plt)
...
plt <- tracks(cplt,plt1,plt2,gplt,heights=c(0.8,1,1,1.5)) + 
  scale_x_sequnit(unit="Mb") # + ylim(0,200)
...
fiveKb.overlap <- check_for_snpoverlap(param.df1,peakc.mod.df,
                                       mvdb.egenes,win=5000)
tenKb.overlap <- check_for_snpoverlap(param.df1,peakc.mod.df,
                                      mvdb.egenes,win=10000)
...
plt1a <- capc_plot(id="ADCY5",experiment="promoter",plotwin=2.1e5)
plt2a <- capc_plot(id="CAMK1D",experiment="promoter",plotwin=2.5e5)
plt3a <- capc_plot(id="STARD10",experiment="promoter",plotwin=2.5e5)
...
oneKb.overlap2 <- check_for_snpoverlap(param.df1,peakc.mod.df,
                                       egene.vec,win=1000)
tenKb.overlap2 <- check_for_snpoverlap(param.df1,peakc.mod.df,
                                       egene.vec,win=10000)
...
plt6a <- credset_plot(id="ADCY5",experiment="promoter",
                     plotwin=2.5e5,promrefsnps=TRUE)
plt6b <- capc_plot(id="ADCY5",experiment="promoter",plotwin=2.5e5)
plt6c <- capc_plot(id="ADCY5__24DF__cv__chr3_123065778",experiment="enhancer",plotwin=2.5e5)
...
plt7a <- credset_plot(id="CAMK1D",experiment="promoter",
                     plotwin=2.5e5,promrefsnps=TRUE)
plt7b <- capc_plot(id="CAMK1D",experiment="promoter",plotwin=2.5e5)
plt7c <- capc_plot(id="rs11257658_CAMK1D__5QTL__eQTL__chr10_12309268",experiment="enhancer",plotwin=2.5e5)
...
plt8a <- credset_plot(id="STARD10",experiment="promoter",
                     plotwin=2.5e5,promrefsnps=TRUE)
plt8b <- capc_plot(id="STARD10",experiment="promoter",plotwin=2.5e5)
plt8c <- capc_plot(id="ARAP1__99DF__cv__chr11_72432985",experiment="enhancer",plotwin=2.5e5)
