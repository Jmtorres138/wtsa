...
plot.dir <- serv.dir1 %&% "projects/wtsa/enhancer_driven/first_round/statistical_anlayses/plots/"
...
chromHMM_plot <- function(state.df,params,plotwin){
  ...
  plt <- ggplot(data=sub.df) + 
    ...
    #coord_cartesian(ylim=c(-2,8),expand = TRUE) + 
    theme_clear() + 
    theme(#axis.title = element_blank(),
          axis.text.y = element_blank(),
          axis.ticks.y=element_blank()) + 
    #annotate("text",x=params$Frag_start,y=6.7,label="Islet") + 
    #annotate("text",x=params$Frag_start,y=4.7,label="ES-HUES6") + 
    #annotate("text",x=params$Frag_start,y=2.7,label="ES-HUES64") + 
...
save_plot <- function(id,plotwin=500000){
  ...
  bdg.df <- fread(bg.dir %&% id %&% ".unionbdg") %>% as.data.frame(.)
  ...
  plt1 <- capc_plot(bdg.df=bdg.df,d=plt.list[[1]],to.exclude=plt.list[[2]],
                 params=plt.list[[3]], peakc.res=peakc.res, plotwin) + scale_x_sequnit(unit="kb")
...
#HHEX – 400 kb, only variant 94462882 & 94480334

#DGKB – 1 MB

#And if you have time, if you can prepare the extension for RREB1 to 400 kb to.

 

#Also, something I forgot to ask you. Would it be possible for you to visualise the promoter captures in CaptureCompare? Would be interesting for ADCY5 and WARS where I am compiling the promoter and enhancer capturesJ
...
gene.vec <- c("ADCY5","COBLL","HHEX","DGKB","RREB1")
...
for (g in gene.vec){
  for (f in file.vec){
    if (grepl(g,f)==TRUE){
      ...
      id.vec <- append(id.vec,strsplit(x=f,split=".unionbdg")[[1]][1])
    }
  }
}


id.vec <- c("RREB1__2TG__DIAMANTE_Morris__chr6_7231843", 
            "rs17168486_DGKB__7QTL__eQTL__chr7_14898282",
            "rs10231021_DGKB__6QTL__eQTL__chr7_15060429",
            "HHEX__89DF__credible_variant__chr10_94462882",
            "HHEX__89DF__credible_variant__chr10_94466439",
            "HHEX__89DF__credible_variant__chr10_94480334",
            "HHEX__89DF__downstream_control__chr10_94602920",
            "HHEX__89DF__upstream_control__chr10_94353285")
