...
stat_dir = "/well/mccarthy/users/jason/projects/wtsa/enhancer_driven/first_round/statistical_anlayses/"
out_dir = stat_dir + "output_files/"
ref_dir = stat_dir + "reference_files/"
genome_file = ref_dir + "hg19.chrom.sizes"


def get_intersect(query_bed,capc_bed,temp_name):
	command = ["bedtools", "intersect", "-wa","-a",query_bed,"-b",capc_bed,"|","uniq",">",out_dir+"inter.temp."+temp_name+".bed"]
...
def enrich(fname,query_bed):
	...
	observed = int(get_intersect(query_bed,out_dir+"temp."+temp_name+".bed",temp_name))
	iter_list = []
	for i in range(1,1001):
		...
		sp.check_call(" ".join(command),shell=True)
		rand_count = get_intersect(query_bed,out_dir+"shuffle.temp."+temp_name+".bed",temp_name)
...
def main():
	file_list = ["peakC-interactions-default_withBaits.txt"]
	...
	"PDX1.bed","islet_specific_strong_enhancer.bed","adipose_specific_strong_enhancer.bed",
