---
title: "Plotting credible set regions "
author: "Jason Torres"
date: "June 4, 2017"
...
gcred_plot <- function(segnum,fac=10){
  ...
  gen.df <- filter(gcred.df,CHR==as.numeric(gsub("chr","",chrom)),POS >= mn & POS <=mx) %>% arrange(desc(PPA))
  if (dim(gen.df)[1]==0){ # single variant, where mn = mx 
    for (locus in unique(gcred.df$LOCUS)){
      temp.df <- filter(gcred.df,LOCUS==locus)
