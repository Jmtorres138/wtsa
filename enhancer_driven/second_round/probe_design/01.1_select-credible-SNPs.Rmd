...
There are 159 credible SNPs (from 120 loci) that have PPA >=0.10 and overlap islet ATAC peaks (MACS2 on n=100 islet ATAC samples) using filtered ATAC peaks 

There are 665 credible SNPs (from 280 loci) that have PPA >=0.01 and overlap islet ATAC peaks (MACS2 on n=100 islet ATAC samples) using filtered ATAC peaks
...
write.table(x=new.df,file=work.dir %&% "second-round_T2D-SNPs_PPA01.txt",sep="\t",quote=F,row.names=F)

```

There are 139 SNPs with PPA > 10% at 108 loci that meet the above criteria and have not been captured in the first round 
There are 621 SNPs with PPA > 1% at 267 loci that meet the above criteria and have not been captured in the first round 
