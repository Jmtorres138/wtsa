...
gcred_plot <- function(segnum,fac=10){
  ...
  gen.df <- filter(gencred.df,CHR==as.numeric(gsub("chr","",chrom)),POS >= mn & POS <=mx) %>% arrange(desc(PPA))
  if (dim(gen.df)[1]==0){ # single variant, where mn = mx 
    for (locus in unique(gencred.df$LOCUS)){
      ...
      if (as.numeric(gsub("chr","",chrom))==lchr & mn >= lcmn & mn <= lcmx){
        gen.df <- arrange(temp.df,desc(PPA))
      }
    }
  }
  plt <- ggplot(data=gen.df,aes(x=POS,y=PPA)) + 
    ...
    scale_y_continuous(breaks=seq(0,1,0.2),limits=c(0,1)) + 
    coord_cartesian(ylim=c(0,1),expand=TRUE)
  plt <- plt + geom_label_repel(data=arrange(gen.df,desc(PPA))[1:1,],
                                aes(x=POS,y=PPA,label=SNPID),size=3)
  return(plt)
}
