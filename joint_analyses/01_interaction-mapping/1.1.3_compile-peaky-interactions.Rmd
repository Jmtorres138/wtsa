...
consolidate_experiment_results <- function(cc.file,peaky.file.dir,exper="promoter"){
  ...
  for (i in 1:length(peaky.file.vec)){
    ...
    print(peaky.file)
    sig.df <- extract_sig_interactions(cc.df,peaky.file,experiment=exper,
                                        celltype="Endo")
...
write.table(x=prom.list[[1]],file=peaky.dir%&%"promoter_endo_peaky-interactions_unmerged-full-info.txt",
            sep="\t",quote=F,row.names=F)
write.table(x=prom.list[[2]],file=peaky.dir%&%"promoter_endo_peaky-interactions.txt",
            sep="\t",quote=F,row.names=F)
write.table(x=prom.list[[3]],file=peaky.dir%&%"promoter_endo_peaky-interactions_merge1K.txt",
            sep="\t",quote=F,row.names=F)
```
