...
build_interaction_dfs <- function(experiment.name,experiment.dir,capture,
  ...
  if (dim(bg.df)[1]>=1000){ # Capture must contain at least 1000 fragments in unionbedgraph 
    ...
    if (dim(peak.df)[1]>0){
      ...
      out.df <- peak.df
      out.df$seqnames <- purrr::map(peak.df$fragment,function(s){
        strsplit(x=s,split=":")[[1]][1]
      }) %>% as.character(.)
      out.df$start <- purrr::map(peak.df$fragment,function(s){
        (strsplit(x=s,split=":")[[1]][2] %>% strsplit(.,split="-"))[[1]][1]
      }) %>% as.integer(.)
      out.df$end <- purrr::map(peak.df$fragment,function(s){
...
build_experiment_interaction_dfs <- function(experiment.name="promoter",experiment.dir=prom.cc.dir,
  ...
  for (i in 1:dim(ex.df)[1]){
    ...
    chrom <- ex.df$V2[i]
    vp <- ex.df$V3[i]
    build.df <- build_interaction_dfs(experiment.name,experiment.dir,capture,
...
prom.df <- build_experiment_interaction_dfs(experiment.name="promoter",experiment.dir=prom.cc.dir,
                                             experiment.file=prom.cc.file,
                                             celltype1="Endo",celltype2="LCL")
prom.df.full <- prom.df %>% get_dist_from_vp(.,experiment.file=prom.cc.file)
prom.df.full$type <- ifelse(prom.df.full$dist.from.vp <= 2.5e+05,"proximal","distal")
write.table(x=prom.df.full,file=output.dir%&%"promoter_endo_DESeq2-interactions.txt",
...
```{r}
e1.df <- build_experiment_interaction_dfs(experiment.name="enhancer.1st",experiment.dir=e1.cc.dir,
                                             experiment.file=e1.cc.file,
                                            celltype1="Endo",celltype2="hESC")
e1.df.full <- e1.df %>% get_dist_from_vp(.,experiment.file=e1.cc.file)
e1.df.full$type <- ifelse(e1.df.full$dist.from.vp <= 2.5e+05,"proximal","distal")
...
```{r}
e2.df<- build_experiment_interaction_dfs(experiment.name="enhancer.2nd",experiment.dir=e2.cc.dir,
                                             experiment.file=e2.cc.file,
                                            celltype1="Endo",celltype2="hESC")
e2.df.full <- e2.df %>% get_dist_from_vp(.,experiment.file=e2.cc.file)
e2.df.full$type <- ifelse(e2.df.full$dist.from.vp <= 2.5e+05,"proximal","distal")
write.table(x=e2.df.full,file=output.dir%&%"enhancer-secondRound_DESeq2_peakC-interactions.txt",
            sep="\t",quote=F,row.names=F)
```

NOTE: captures corresponding to these indices initially failed without the 1Mb range constraint: 
25;27;36;56;60;65;75;106;119;163;171;172;
