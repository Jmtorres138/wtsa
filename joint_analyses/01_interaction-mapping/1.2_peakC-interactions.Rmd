...
build_experiment_interaction_df <- function(experiment.name="promoter",experiment.dir=prom.cc.dir,
  ...
  for (i in 1:dim(ex.df)[1]){
    ...
    chromo <- ex.df$V2[i]
    vp <- ex.df$V3[i]
    build.prox <- build_interaction_df(experiment.name,experiment.dir,
                                        capture,vp,chromo,range=2.5e+05,celltype)
    build.dist <- build_interaction_df(experiment.name,experiment.dir,
                                        capture,vp,chromo,range=1e+06,celltype)
    if (!is.null(build.dist)){
      check <- extract_distal(vp,build.dist)
      if (is.null(check)){
        build.dist <- NULL
      } else{
        build.dist <- check
...
```{r}
prom.df <- build_experiment_interaction_df(experiment.name="promoter",
  experiment.dir=prom.cc.dir,                                 
  experiment.file=prom.cc.file,celltype="Endo")
prom.full.df <- prom.df %>% get_dist_from_vp(.,experiment.file=prom.cc.file)
write.table(x=prom.full.df,file=output.dir%&%"promoter_endo_peakC-interactions.txt",
...
```{r}
e1.df <- build_experiment_interaction_df(experiment.name="enhancer.1st",experiment.dir=e1.cc.dir,
                                             experiment.file=e1.cc.file,celltype="Endo")
e1.df <- e1.df %>% get_dist_from_vp(.,experiment.file=e1.cc.file)
...
```{r}
e2.df <- build_experiment_interaction_df(experiment.name="enhancer.2nd",
  experiment.dir=e2.cc.dir,
  experiment.file=e2.cc.file,celltype="Endo")
e2.full.df <- e2.df %>% get_dist_from_vp(.,experiment.file=e2.cc.file)
write.table(x=e2.full.df,file=output.dir%&%"enhancer-secondRound_endo_peakC-interactions.txt",
            sep="\t",quote=F,row.names=F)
```
