...
build_experiment_interaction_dfs <- function(experiment.name="promoter",experiment.dir=prom.cc.dir,
  ...
  for (i in 1:dim(ex.df)[1]){
    ...
    chromo <- ex.df$V2[i]
    vp <- ex.df$V3[i]
    build.list.prox <- build_interaction_dfs(experiment.name,experiment.dir,
                                        capture,vp,chromo,range=2.5e+05,celltype)
    build.list.dist <- build_interaction_dfs(experiment.name,experiment.dir,
                                        capture,vp,chromo,range=1e+06,celltype)
    if (!is.null(build.list.dist)){
      check1 <- extract_distal(vp,build.list.dist[[1]])
      check2 <- extract_distal(vp,build.list.dist[[2]])
      if (is.null(check1)|is.null(check2)){
        build.list.dist <- NULL
      } else{
        build.list.dist <- list(check1,check2)
      }      
    }
    if (!is.null(build.list.prox)){
      build.list.prox[[1]]$type <- "proximal";build.list.prox[[2]]$type <- "proximal"
      out.df1 <- rbind(out.df1,build.list.prox[[1]])
      out.df2 <- rbind(out.df2,build.list.prox[[2]]) 
    }
    if (!is.null(build.list.dist)){
      build.list.dist[[1]]$type <- "distal";build.list.dist[[2]]$type <- "distal"
      out.df1 <- rbind(out.df1,build.list.dist[[1]])
      out.df2 <- rbind(out.df2,build.list.dist[[2]]) 
    }
...
e2.list <- build_experiment_interaction_dfs(experiment.name="enhancer.2nd",
