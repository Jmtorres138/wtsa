...
caqtl.chrom.dir <- coloc.dir %&% "rasqual_output/with_p_values/"
args <- commandArgs(trailingOnly=TRUE)
chromo <- args[1]

caqtl.df <- fread(caqtl.chrom.dir %&% "results_rasqual_" %&%
                    chromo %&% "_with_Pvalues.txt.gz")
  ...
  if (dim(caqtl.sub.df)[1]>1){
    ...
    for (i in 1:length(peak.vec)){
      ...
      if (length(maf.vec)>0 & length(maf.vec)==length(p.vec.gwas) &
        ...
        print("Not enough information present for Peak to run COLOC: " %&% peak)
      }
    }
  } else{
    print("Not sufficient caQTL overlap for signal: " %&% gwas.signal)
    out.name <- out.dir %&% gwas.signal %&% ":" %&% "FAILED" %&% ".coloc.RDS"
    saveRDS(object=gwas.signal,file=out.name)
  }
}
